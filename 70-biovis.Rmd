...
mulvey2015norm <- as(mulvey2015norm, "SummarizedExperiment")
heatmap(assay(mulvey2015norm))
```
...
heatmap(assay(mulvey2015norm), distfun = cor.dist)
```
...
x <- assay(mulvey2015norm)
hcl <- hclust(dist(x))
cl <- cutree(hcl, k = 12)
ha1 <- HeatmapAnnotation(time = mulvey2015norm$times)
...
library("pRolocdata")
data(hyperLOPIT2015)
hl <- as(hyperLOPIT2015, "SummarizedExperiment")
## interactive heatmap
library("heatmaply")
heatmaply(assay(hl)[1:100, ])
```

```{r heatmaply2, eval = FALSE}
heatmaply(assay(hl)[1:100, ],
          RowSideColors = as.factor(rowData(hl)$markers[1:100]))
```
...
`r CRANpkg("venneuler")` and `r Githubpkg("js229/Vennerable")`[^vennerable].

[^vennerable]: You can install `Vennerable` with
    `BiocManager::install("js229/Vennerable")`.

We will use the `mulvey2015norm` feature names to generate a test data:

```{r venndat}
set.seed(123)
feat_list <- replicate(3,
                       sample(rownames(mulvey2015norm), 555),
                       simplify = FALSE)
...
```{r upsetchallenge}
set.seed(123)
feat_list_10 <- replicate(10, sample(rownames(mulvey2015norm), 555), simplify = FALSE)
