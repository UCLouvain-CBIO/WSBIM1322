...
library("pRolocdata")
data(mulvey2015norm_se)
heatmap(assay(mulvey2015norm_se))
```
...
heatmap(assay(mulvey2015norm_se), distfun = cor.dist)
```
...
```{r d3hm, eval = FALSE}
library("d3heatmap")
d3heatmap(assay(hyperLOPIT2015_se))
```
...
```{r upsetchallenge}
set.seed(123)
feat_list_10 <- replicate(10, sample(rownames(mulvey2015norm_se), 555), simplify = FALSE)
