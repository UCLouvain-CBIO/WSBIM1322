...
```{r corfig, fig.cap = "Example of correlation, anti-correlation and lack thereof.", echo = FALSE, fig.width = 12}
x <- rnorm(60)
y1  <- x + rnorm(60, 0, 0.3)
y2 <- -y1
y3 <- rnorm(60)
df <- data.frame(x = c(x, x, x),
           y = c(y1, y2, y3),
           type = rep(c("correlation", "anti-correlation",
                        "no correlation"), each = 60))
