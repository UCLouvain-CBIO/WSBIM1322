...
`r msmbstyle::solution_begin()`
```{r, fig.cap = "Visualisation of the labelled and unlabelled data before (top) and after (bottom) classification.", fig.height = 10, warning = FALSE}
giris2_res <- giris2
...
will, but is unable to provide a precise answer. Ideally, we want
...
Compare you results with those that were obtained using *support
vector machine*, a popular classifier that was used in Christoforou
*et al.* [@Christoforou:2016], and available in the
`svm.classification` feature variable.
