...
`giris2_train`, and a second one containing the unlabelled data called
`giris_test`.
...
2. the unlabelled preductors, on which the model will be applied (see
...
interprete the results based on the firt PCA.
`r msmbstyle::question_end()`

`r msmbstyle::solution_begin()`
```{r, fig.cap = "Visualiation of the labelled and unlabelled data before (top) and after (bottom) classification.", fig.height = 10, warning = FALSE}
giris2_res <- giris2
giris2_res[is.na(giris2$GRADE), 5] <- knn_res
pca_knn <- prcomp(giris2_res[, -5], scale = TRUE)
p1 <- factoextra::fviz_pca_ind(pca2, col.ind = giris2$GRADE)
p2 <- factoextra::fviz_pca_ind(pca_knn, col.ind = giris2_res$GRADE)
library("patchwork")
p1 / p2
```
`r msmbstyle::solution_end()`
...
proceding as above. To be able to make an informed decision about the
**model parameter** k, we need to to measure the **performance** of
the kNN clasifier for different values of k. Do do so, we are going to
...
There are numerous different ways to measure the performace of a
...
to generate a contingency matrix (using the `table` function) that
tallies matches and mis-matches betweem the predictions and expected
assignments. Interprete these results.
`r msmbstyle::question_end()`
...
table(test_1, giris2_test[, 5])
table(test_11, giris2_test[, 5])
```
`r msmbstyle::solution_end()`
...
1. Test all odd value of k from 1 to 11 (or higher, if deemed useful),
   to have a better granularity of the model paramter we test.
...
set of size around $\frac{n (k - 1)}{k}$ and a testing set of size arond $\frac{n}{k}$.
...
knn_accuracy <- function(fld, k = 11) {
    giris2_train <- giris2_labelled[-fld, ]
    giris2_test <- giris2_labelled[fld, ]
    test_k <- knn(giris2_train[, -5],
                  giris2_test[, -5],
                  giris2_train[, 5],
                  k)
    sum(test_k == giris2_test[, 5])/length(test_k)
}
...
due to some instances that lie at the boudary between classes B and C.

There exist a variety of packages that automate model and parameter
selection using cross-validation procedures as illustrated above, so
that in practice, these tasks can be automated. This becomes
particularly useful when more than one model parameter (called
hyper-parameters) need to tuned, different classifiers need to be
assessed, and/or several model performance metrics are to be computed.

## Variance and biais tradeoff

A recurrent goal in machine learning is to find a trade-off between
variance and biais when fitting models. It is important to build a
...
to build it (the model would be said to be **overfitting the data**)
...
This variance and biais tradeoff can be illustrated in different
...
This last figure, reproduced from [@MSMB],

```{r, echo = FALSE, fig.cap = "Precision and accuracy when shooting."}
par(mar = c(0, 1, 0, 1))
plot(1:15, 1:15,
     type = "n", xaxt = "n", yaxt = "n", xlab = "",
     ylab = "", bty = "n")
plotrix::draw.circle(4, 7, 3, col = "steelblue")
plotrix::draw.circle(4, 7, 2, col = "red")
plotrix::draw.circle(4, 7, 1, col = "yellow")
set.seed(12)
x <- rnorm(20, 4, 1.2)
y <- rnorm(20, 7, 1.2)
points(x, y)
points(mean(x), mean(y), pch = 19)
plotrix::draw.circle(12, 7, 3, col = "steelblue")
plotrix::draw.circle(12, 7, 2, col = "red")
plotrix::draw.circle(12, 7, 1, col = "yellow")
set.seed(12)
x <- rnorm(20, 13, 0.4)
y <- rnorm(20, 8, 0.4)
points(x, y)
