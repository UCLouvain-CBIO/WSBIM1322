...
substrings of a sequence. This can be done by 

- creating a new `DNAStringSet` (or `DNAString` for only one sequence)
  object and specifying start or end positions, or their width or
  nchar:

```{r}
DNAStringSet(dna, start = 1, end = 100)
DNAStringSet(dna, start = 1, width = 100)
DNAString(dna[[1]], start = 1, nchar = 10)
```

- or by using the `subseq` function:
