...
letterFrequency(chr14_dna, c("A", "G", "C", "T"), as.prob = TRUE)
```
`r msmbstyle::solution_end()`
...
`r msmbstyle::question_begin()`
Focusing on the region from base 100 million to 101 million along chromosome
14, calculate the GC content across a sliding window of 1000
nucleotides (you can use the `letterFrequencyInSlidingView` functionn)
and visualise it (see an example below). On that plot, show the mean
GC content and highlight the regions that have a GC content > 0.7 and
< 0.3.
`r msmbstyle::question_end()`


```{r, echo = FALSE, fig.cap = "GC content for chomosome 14 (100 - 101 Mb)"}
x <- subseq(chr14_dna[[1]], 100 * 1e6, 101 * 1e6)
gc_content <- letterFrequencyInSlidingView(x, 1e3,
                                           letters = "GC",
                                           as.prob = TRUE)
plot(gc_content, type = "l",
     main = "Chromosome 14: 100 - 101 Mb")
abline(h = mean(gc_content))
abline(v = which(gc_content > 0.7), col = "#FF0000")
abline(v = which(gc_content < 0.3), col = "#0000FF")
```
...
above. To to so, you will need to match the matching protein
identifiers from that vector and the names of the `AAStringSet`
above.
`r msmbstyle::question_end()`

```{r pattern2, message = FALSE, echo = FALSE, include = FALSE}
library("rWSBIM1322")
data(up_selected)
head(up_selected)
head(names(up))
...
the `str_detect` function from the `stringi` package.
`r msmbstyle::question_end()`



```{r pattern3, echo = FALSE, include = FALSE}
library("stringi")
...
```{r pattern4, echo = FALSE, include = FALSE}
