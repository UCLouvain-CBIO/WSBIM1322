...
`r ncol(mulvey2015)` samples from a `r nrow(mulvey2015)`-dimensional space into 2, 
...
`r msmbstyle::question_begin()`
Load the `hyperLOPIT2015` data from the `pRolocdata` package. Check its
class and familiarise yourself with its feature annotation stored in the
*featureData* slot that can be accessed with the `fData` function.
`r msmbstyle::question_end()`
...
library(pRolocdata)
data(hyperLOPIT2015)
fData(hyperLOPIT2015)[, c("entry.name", 
```
`r msmbstyle::solution_end()`
