...
Note that dendrograms, or trees in general, are used in evolutionary
biology to visualise the evolutionary history of taxa. In this course,
we use them to represent similarities (based on distances) and don't
assess any evolutionary relations between features or samples.
...
- Load the `mulvey2015` data from the `pRolocdata` package. The goal
  of this exercise is to identify proteins that haev a specific
...
ms2df(mulvey2015, fcol = "cl") %>%
    ...
    gather(key = sample, value = expression, -cl, -protein) %>%
