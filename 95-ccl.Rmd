...
`r msmbstyle::question_begin()`
Log-2 transform the expression data and visualise the distributions of
the expression values in each samples.
`r msmbstyle::question_end()`

`r msmbstyle::solution_begin()`

```{r, fig.cap = "Expression values in cell types A and B without (black and gree) and with stimulation (red and blue).", warning = FALSE}
e <- log2(e + 1)
boxplot(e, col = as.factor(pd$group))
```
`r msmbstyle::solution_end()`

Continue without preforming any normalisation.

`r msmbstyle::question_begin()`
Removes genes that aren't expressed in any samples, i.e. those that
have a sum of expression equal to 0 accross all samples. How many
genes unexpressed genes are there?
`r msmbstyle::question_end()`
...
`r msmbstyle::question_begin()`
Visualise and interpret the experiment desing (cell type and
treatment) using a principal component analyis.
`r msmbstyle::question_end()`

`r msmbstyle::solution_begin()`
```{r, fig.cap = "PCA of *kem2* samples."}
library("factoextra")
...
between the stimulated and non-stimulated samples of cell type A.
`r msmbstyle::question_end()`

`r msmbstyle::solution_begin()`
We start by selecting the relevant samples and create a new expression
matrix matching cell type A. Visualise the results on a volcano plot.
```{r}
...
```{r, fig.cap = "Histogram of p-values, showing an enrichment of small values."}
...
```{r, fig.cap = "Volcano plot illustrating how stimulation down-regulates the expression of numerous genes."}
...
`r msmbstyle::solution_begin()`
```{r, fig.cap = "Heatmap of the 73 differentially expressed genes."}
k_de <- res %>%
    rownames_to_column() %>%
    filter(adjp < 0.001, abs(lfc) > 1) %>%
...
```{r, message = FALSE, fig.cap = "Annotated heatmap of the 73 differentially expressed genes."}
...
`r msmbstyle::solution_begin()`
```{r, fig.cap = "Expression of the three most differentially expressed genes."}
k_3 <- res %>%
    rownames_to_column() %>%
    arrange(adjp) %>%
    head(3) %>%
...
```{r, fig.cap = "PCA of the *kem2* genes displaying the 5 arbitrary clusters."}
set.seed(1)
...
`r msmbstyle::solution_begin()`
```{r, fig.cap = "Volcano plot displaying the five clusters"}
