...
comes as an object of class `MSnSet`, which you can convert to a
`SummarizedExperiment` as shown below. Familiarise yourself with its
experimental design stored in the *rowData* slot.
...
fviz_pca_ind(pca_mulvey2015, habillage = mulvey2015$time)
```
`r msmbstyle::solution_end()`
...
p <- prcomp(t(assay(x)), scale = TRUE, center = TRUE)
