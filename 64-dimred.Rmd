...
Load the `cptac_prot` data[^msnset1], scale the data (see below for
...
[^msnset1]: The [`MSnSet`
    ...
    (defined in the `MSnbase` package) is similar to
    `SummarizedExperiment`, that we have seen in detail in chapter
    \@ref(sec-obj). You can convert an `MSnSet` object to a
    `SummarizedExperiment` with `as(x, "SummarizexExperiment")`.
...
comes as an object of class `MSnSet`[^msnset2], which you can convert to a
`SummarizedExperiment` as shown below. Familiarise yourself with its
experimental design stored in the *colData* slot.

[^msnset2]: The [`MSnSet`
    class](https://lgatto.github.io/MSnbase/reference/MSnSet-class.html)
    is similar to the `SummarizedExperiment` class, that we have seen
    in detail in chapter \@ref(sec-obj).
